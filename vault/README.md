...
- **`world-setting/`** — curated, human-edited canon. This is what actually
  becomes Pomisuke's system prompt. The bot starts at `world-setting/index.md`
  and follows its `[[wikilinks]]` **one level deep** — so a new note only
  affects the bot's behavior once you link it from `index.md`. No other note
  is auto-included, and there's no deeper traversal.
- **`auto-log/facts.md`** — bot-written, lower-trust. Whenever Pomisuke
  improvises a new fact about itself or its world mid-conversation, it gets
  appended here automatically as a dated entry. Recent entries are included
  in the prompt as loose context (so Pomisuke doesn't immediately contradict
  itself), but nothing here is treated as canon. Periodically review this
